    ...
    const markdown = toMarkdown(board, {
        itemMapping: (/** @type {ProjectBoardItem} **/item) => {
            ...
            if (hasCrossReferenceLabel) {
                ...
                const matchedReferURL = item.body.match(/https:\/\/github\.com\/(?<owner>.*?)\/(?<repo>.*?)\/(?<type>(issues|pull))\/(?<number>\d+)/);
                if (!matchedReferURL) {
                    return item;
                }
...
module.exports = {
    config: {
        ...
        includes_note: {
            title: 'Includes note',
            description:
                "Push/Pull includes note as a target",
            type: 'boolean',
        },
        auto_push_on_save: {
            title: 'Auto push on saving',
            description:
                "When change the note, push the changes to GitHub automatically",
            type: 'boolean',
        }
    },
    activate() {
